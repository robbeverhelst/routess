...
describe("Users Integration Tests", () => {
	...
	describe("GET /users/:id", () => {
		it("should return user by id", async () => {
			const response = await supertest(app.getHttpServer()).get(`/api/v1/users/${testUser.id}`).expect(200);
	...
	describe("PATCH /users/:id", () => {
		...
		it("should require authentication", async () => {
			await supertest(app.getHttpServer()).patch(`/api/v1/users/${testUser.id}`).send({ name: "No Auth" }).expect(401);
		});
	});
	...
	describe("User Data Consistency", () => {
		it("should maintain data consistency when user has routes", async () => {
			...
			const userResponse = await supertest(app.getHttpServer()).get(`/api/v1/users/${testUser.id}`).expect(200);
