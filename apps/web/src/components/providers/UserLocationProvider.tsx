...
interface UserLocationContextType {
  ...
  startTracking: () => void;
  stopTracking: () => void;
  getCurrentLocation: (options?: any) => Promise<{
    location: [number, number] | null;
    accuracy: number | null;
    error: string | null;
  }>;
...
export const UserLocationProvider: React.FC<UserLocationProviderProps> = ({
  ...
  useEffect(() => {
    if (isLocationTracking && locationError) {
      const timer = setTimeout(() => {
        if (isLocationTracking && locationError) {
          Logger.info(
            "[UserLocationProvider] Auto-stopping location tracking due to persistent errors",
          );
  ...
  return (
    <UserLocationContext.Provider value={contextValue}>{children}</UserLocationContext.Provider>
  );
};
