...
try {
  ...
  if (storedData) {
    ...
    if (parsed && parsed.waypoints && parsed.waypoints.length > 0) {
      ...
      Logger.info(
        "[MapWithRouting Init] Detected route in localStorage on component initialization.",
      );
    }
  }
} catch (e) {
...
try {
  ...
  Logger.error(
    "[MapWithRouting Init] Error reading lastKnownLocation from localStorage on init:",
    e,
  );
}
...
function findNearestPointOnPolyline(
...
): { point: [number, number] | null; distance: number | null } {
  ...
  return {
    point: overallClosestPoint,
    distance: minDistanceFound === Infinity ? null : minDistanceFound,
  };
}
...
export default function MapWithRouting({
  ...
  ); // Initialize with localStorage or default to 'day'
  ...
  const [showSunDirection, setShowSunDirection] = useState<boolean>(
    loadSunDirectionSettingFromLocalStorage(),
  );
  ...
  useEffect(() => {
    if (userLocation && !currentSunPosition) {
      ...
      Logger.info(
        `[MapWithRouting] Initial sun position calculated: azimuth=${sunPos.azimuth.toFixed(1)}°, elevation=${sunPos.elevation.toFixed(1)}°`,
      );
    }
  ...
  const handleMapLoad = useCallback(
    async (event: { target: mapboxgl.Map }) => {
      ...
      if (mapRef.current.getLayer(ROUTE_CASING_LAYER_ID)) {
        mapRef.current.setPaintProperty(
          ROUTE_CASING_LAYER_ID,
          "line-color",
          isDarkMode ? NIGHT_ROUTE_CASING_COLOR : DAY_ROUTE_CASING_COLOR,
        );
        mapRef.current.setPaintProperty(
          ROUTE_CASING_LAYER_ID,
          "line-opacity",
          isDarkMode ? NIGHT_ROUTE_CASING_OPACITY : DAY_ROUTE_CASING_OPACITY,
        );
      }
      if (mapRef.current.getLayer(WAYPOINTS_LAYER_ID)) {
        mapRef.current.setPaintProperty(WAYPOINTS_LAYER_ID, "circle-color", [
          ...
          ["get", "pointType"],
          "start",
          isDarkMode ? NIGHT_WAYPOINT_START_COLOR : DAY_WAYPOINT_START_COLOR,
          "end",
          isDarkMode ? NIGHT_WAYPOINT_END_COLOR : DAY_WAYPOINT_END_COLOR,
          "direct",
          isDarkMode ? NIGHT_WAYPOINT_DIRECT_COLOR : DAY_WAYPOINT_DIRECT_COLOR,
          isDarkMode ? NIGHT_WAYPOINT_DEFAULT_COLOR : DAY_WAYPOINT_DEFAULT_COLOR,
        ]);
        mapRef.current.setPaintProperty(
          WAYPOINTS_LAYER_ID,
          "circle-stroke-color",
          isDarkMode ? NIGHT_WAYPOINT_STROKE_COLOR : DAY_WAYPOINT_STROKE_COLOR,
        );
      }
      if (mapRef.current.getLayer(ROUTE_ARROWS_LAYER_ID)) {
        mapRef.current.setPaintProperty(
          ROUTE_ARROWS_LAYER_ID,
          "text-color",
          isDarkMode ? NIGHT_ROUTE_ARROW_TEXT_COLOR : DAY_ROUTE_ARROW_TEXT_COLOR,
        );
        mapRef.current.setPaintProperty(
          ROUTE_ARROWS_LAYER_ID,
          "text-halo-color",
          isDarkMode ? NIGHT_ROUTE_ARROW_HALO_COLOR : DAY_ROUTE_ARROW_HALO_COLOR,
        );
      }
      ...
      if (routeDataParam) {
        ...
        if (loadedData && mapRef.current && MAPBOX_TOKEN) {
          ...
          Logger.warn(
            "[MapWithRouting] Failed to process route data from URL (e.g. map not ready, token missing, or data invalid but not throwing). RouteDataParam was present.",
          );
          // Avoid showing a generic error if a specific one was already shown by the catch block for decompressAndParse
          if (loadedData !== null) {
            // Only show this if decompressAndParse didn't fail and show its own error
            handleRouteInfoErrorFromHook(
              "Could not load shared route. The link appears to be invalid or data is missing.",
            );
          }
        }
      }
  ...
  useEffect(() => {
    ...
    if (hasRoute && isRouteCoordsReady) {
      ...
      if (currentRouteCoords && currentRouteCoords.length > 0) {
        Logger.info(
          "[MapWithRouting] Route coordinates available from localStorage, zooming to route",
        );
        zoomToRoute(mapRef.current, currentRouteCoords);
        initialRouteZoomDoneRef.current = true;
        hasInitiallyZoomedToUser.current = true;
      ...
      Logger.info(
        "[MapWithRouting] Route detected in localStorage, waiting for route data to be ready",
      );
      return;
    }
    ...
    Logger.info(
      "[MapWithRouting] Priority 4: Using default location (already set in initialViewState)",
    );
  }, [
    isMapReady,
    hasRoute,
  ...
  const handleGenerateCustomRoute = useCallback(
    async (params: RouteGenerationParams) => {
      ...
      if (
        ...
        Logger.info(
          `[MapWithRouting] Attempting to generate A-to-B route from ${params.startPoint.name} to ${params.endPoint.name} with surface ${params.surfaceType}`,
        );
        ...
        try {
          ...
          Logger.error(
            "[MapWithRouting] Error during A-to-B custom route generation attempt:",
            error,
          );
          handleRouteInfoErrorFromHook(
            typeof error === "string"
              ? error
              : "Failed to generate A-to-B route. Please try again.",
          );
          setIsRouteCoordsReady(false);
        } finally {
        ...
        Logger.info(
          "[MapWithRouting] Resetting existing route before attempting loop generation...",
        );
        resetRouting(mapRef.current, setRouteDistance, setRouteDuration, setHasRoute);
        clearShareState();
        setWaypointError(null);
        ...
        Logger.info(
          `  Start: ${params.startPoint.name} (${params.startPoint.lat.toFixed(5)}, ${params.startPoint.lng.toFixed(5)})`,
        );
        ...
        try {
          ...
          Logger.error(
            "[MapWithRouting] Error during loop custom route generation attempt:",
            error,
          );
          handleRouteInfoErrorFromHook(
            typeof error === "string" ? error : "Failed to generate loop route. Please try again.",
          );
          setIsRouteCoordsReady(false);
        } finally {
          // Set generating state to false and close modal
        ...
        Logger.warn(
          "[MapWithRouting] Loop generation requested but essential parameters are missing.",
          params,
        );
  ...
  const handleRemoveWaypoint = useCallback(() => {
    if (!mapRef.current || !popup || popup.type !== "remove" || popup.waypointIndex === undefined)
      return;
  ...
  useEffect(() => {
    ...
    if (isMapLocked && userLocation && hasRoute && isRouteCoordsReady) {
      ...
      if (routePath && routePath.length >= 2) {
        const { point: nearestPoint, distance: distanceToRouteKm } = findNearestPointOnPolyline(
          userLocation,
          routePath,
        );

        // Only show line if nearest point is found AND user is > 50m (0.050 km) away
        if (nearestPoint && distanceToRouteKm !== null && distanceToRouteKm > 0.05) {
    ...
    return () => {
      ...
      if (map && map.getStyle()) {
        // map.getStyle() is a way to check if map is still valid
        try {
          if (map.getLayer(LINE_TO_ROUTE_LAYER_ID)) {
            map.removeLayer(LINE_TO_ROUTE_LAYER_ID);
          }
  ...
  const handleCycleTimeOfDay = useCallback(() => {
    if (mapRef.current) {
      ...
      if (userLocation) {
        ...
        setCurrentSunPosition(sunPos);
        Logger.info(
          `[MapWithRouting] Sun position for ${nextLightPreset}: azimuth=${sunPos.azimuth.toFixed(1)}°, elevation=${sunPos.elevation.toFixed(1)}°`,
        );
      }
        ...
        <div className="flex justify-between items-start w-full">
          ...
          <div className="flex flex-col items-end gap-2 flex-grow pointer-events-auto">
            <div className="flex items-center justify-end gap-2 w-full">
              {" "}
              {/* This container ensures LocationSearch can expand */}
              <LocationSearch
                mapboxToken={MAPBOX_TOKEN}
