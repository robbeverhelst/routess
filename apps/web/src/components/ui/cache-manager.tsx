				...
				<div className="space-y-4 sm:space-y-6">
					...
					<Card className="p-3 sm:p-4">
						<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-2">
							...
							<span
								className={`text-sm px-2 py-1 rounded self-start ${
						...
						{isOnline && (
							<div className="text-sm text-gray-600 space-y-1">
								<div>
									{t("cacheManager.connectionType", currentLanguage)}: {networkStatus.effectiveType}
								</div>
								{networkStatus.downlink > 0 && (
									<div>
										{t("cacheManager.speed", currentLanguage)}: {networkStatus.downlink} Mbps
									</div>
								)}
							</div>
						)}
					...
					<Card className="p-3 sm:p-4">
						<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-2">
							...
							{swState.hasUpdate && (
								<Button size="sm" onClick={updateServiceWorker} className="gap-2 self-start">
							...
							<div className="flex flex-col sm:flex-row gap-2">
								...
								{cacheStatus && Object.keys(cacheStatus).length > 0 && (
									...
									>
										...
										{isClearingAll
											? t("cacheManager.clearing", currentLanguage)
						...
						{isCacheLoading ? (
							...
							<div className="space-y-4">
								...
								{Object.keys(cacheStatus).length === 0 ? (
									<p className="text-gray-500 text-center py-4">{t("cacheManager.noCaches", currentLanguage)}</p>
								) : (
												...
												<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 gap-2">
													<div className="min-w-0 flex-1">
														...
														<p className="text-xs text-gray-500 line-clamp-2 sm:line-clamp-1">
															{getCacheTypeDescription(cacheName)}
														</p>
													</div>
													<Button
														size="sm"
													...
													>
														...
														<span className="hidden sm:inline">
															{isClearing === cacheName
																? t("cacheManager.clearing", currentLanguage)
																: t("cacheManager.clear", currentLanguage)}
														</span>
													</Button>
												</div>
												<div className="flex justify-between text-xs text-gray-600">
													<span>
														{cacheInfo.entries} {t("cacheManager.entries", currentLanguage)}
													</span>
							...
							<p className="text-gray-500 text-center py-4">{t("cacheManager.unavailable", currentLanguage)}</p>
						)}
					</Card>

					{/* Offline Features Info */}
					<Card className="p-3 sm:p-4 bg-blue-50 border-blue-200">
						<h3 className="font-medium text-blue-900 mb-2">{t("cacheManager.offlineFeatures", currentLanguage)}</h3>
