...
import {
	...
	ArrowRight,
	ArrowRightCircle,
	ArrowRightLeft, // Added ArrowRightLeft
	ArrowUp,
	Layers,
	Locate,
	Lock,
...
import { type SupportedLanguage, t } from "@/lib/i18n"; // Added
import { useCanRedo, useCanUndo, useHasRoute, useRouteDistance, useWaypoints } from "@/stores/routingStore";
...
const getIconForTimeOfDay = (time: TimeOfDay): React.ElementType => {
	switch (time) {
		...
		case "night":
			return MoonIcon;
		default:
			return SunIcon; // Default to day icon
	}
};

// Helper to get icon and label for current map orientation
const getOrientationIconAndLabel = (
	bearing: number,
...
function RouteControlsComponent({
	...
	const locateButtonClass =
		"bg-white/90 dark:bg-black/80 text-black dark:text-white hover:bg-white/70 dark:hover:bg-black/60 disabled:opacity-50 h-10 w-10";
	...
	if (canLocateCurrent && !isLocationTracking) {
		...
		if (locationAccuracy !== null && locationAccuracy !== undefined) {
			const accuracyText =
				locationAccuracy <= 10
					? t("location.accuracy.high", currentLanguage, {
							accuracy: Math.round(locationAccuracy).toString(),
						})
					: locationAccuracy <= 50
						? t("location.accuracy.medium", currentLanguage, {
								accuracy: Math.round(locationAccuracy).toString(),
							})
						: t("location.accuracy.low", currentLanguage, {
								accuracy: Math.round(locationAccuracy).toString(),
							});
		...
		<TooltipProvider>
			<div className="flex flex-col lg:flex-row items-start lg:items-center gap-2">
					...
					<TooltipContent>
						<p>
							{currentMapStyle === "satellite"
								...
								: userLocation
									? t("routeControls.tooltip.cycleTimeOfDayWithSun", currentLanguage, {
											time: currentTimeOfDay,
										})
									: t("routeControls.tooltip.cycleTimeOfDay", currentLanguage, {
											time: currentTimeOfDay,
										})}
						</p>
					</TooltipContent>
				...
				<Tooltip delayDuration={500}>
					...
					<TooltipContent>
						<p>
							{t("routeControls.tooltip.toggleMapStyle", currentLanguage, {
								style: currentMapStyleName,
							})}
						</p>
					</TooltipContent>
				</Tooltip>
				...
				<Tooltip delayDuration={500}>
					...
					<TooltipContent>
						<p>
							{t("routeControls.tooltip.setMapOrientation", currentLanguage, {
