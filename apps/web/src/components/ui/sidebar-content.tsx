...
const LoginModal = React.lazy(() =>
	import("../auth/LoginModal").then((module) => ({
...
export function SidebarContent({
	...
	const { handleExportGPX, handleImportGPX } = useRouteImportExport({
		map,
				...
				<div className="px-3 py-2 border-b border-gray-200 dark:border-gray-800">
					<TooltipProvider>
						<div className="flex items-center justify-center gap-1">
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<Button
										...
										className={`h-8 w-8 ${isLocked ? "text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20" : ""}`}
									>
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<Button variant="ghost" size="icon" onClick={onZoomToRoute} disabled={!hasRoute} className="h-8 w-8">
										<Focus size={16} />
									</Button>
								</TooltipTrigger>
								<TooltipContent side="bottom">
									...
									<div className="w-full px-3 py-3 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
										<div className="flex items-center gap-3">
											...
											<div className="min-w-0 flex-1">
												...
												<p className="text-xs text-gray-600 dark:text-gray-300 truncate mt-0.5">{currentUser?.email}</p>
											</div>
											<button
												type="button"
									...
									<div className="flex items-center justify-between mt-3">
										<div className="flex gap-1">
											{tips.map((tip, index) => (
												<div
													key={tip.title}
													className={`w-1.5 h-1.5 rounded-full transition-opacity ${index === currentTipIndex ? "bg-white" : "bg-white/40"}`}
												/>
											))}
										</div>
