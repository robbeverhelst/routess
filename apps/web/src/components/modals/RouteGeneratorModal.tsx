...
import { BarChart2, ChevronRight, Compass, Loader2, LocateFixed, Map as MapIcon, MapPin, Route, X } from "lucide-react";
	...
	const handleLoopLengthChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (!Number.isNaN(numValue)) {
			if (numValue > 500) {
				...
				// Keep 0 as a possible input during typing, validation on generate button
				setLoopLengthKm(0);
			} else {
			...
			<DialogContent className="p-0 w-[330px] sm:max-w-[525px] sm:w-[525px] border-r" hideCloseButton>
				<div className="border-b border-gray-100 dark:border-gray-800 relative px-4 py-4 pb-3">
					...
					<div className="flex items-center pr-8">
						...
						<DialogTitle className="text-base font-medium m-0 p-0">
							{isGenerating
								? t("routeGenerator.title.generating", currentLanguage)
								: t("routeGenerator.title.default", currentLanguage)}
						</DialogTitle>
						<VisuallyHidden asChild>
							<DialogDescription>{t("routeGenerator.description", currentLanguage)}</DialogDescription>
						</VisuallyHidden>
					</div>
				</div>
				...
				<div className="bg-amber-50 dark:bg-amber-900/20 border-b border-amber-100 dark:border-amber-800/30 px-4 py-3 text-center">
					<p className="text-xs text-amber-800 dark:text-amber-400">
						<span className="font-semibold">{t("routeGenerator.betaNotice.title", currentLanguage)}</span>{" "}
						{t("routeGenerator.betaNotice.message1", currentLanguage)}
					</p>
						...
						<div className="text-center">
							<p className="text-base font-medium mb-1">
								{routeType === "a-to-b"
									...
									: t("routeGenerator.generatingState.loop", currentLanguage, {
											loopLengthKm: loopLengthKm?.toString() || "",
										})}
							</p>
						...
						<div className="px-4 overflow-y-auto max-h-[calc(100vh-120px)]">
							...
							<div className="mt-4">
								<div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
									{t("routeGenerator.routeType.label", currentLanguage)}
								</div>
								<RadioGroup
									value={routeType}
								...
								>
									<Label
										htmlFor="r2"
										className={`flex flex-col items-center justify-center p-3 rounded-md border ${routeType === "loop" ? "bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800" : "border-gray-200 dark:border-gray-800"} cursor-pointer w-full h-full`}
									>
										<RadioGroupItem value="loop" id="r2" className="sr-only" />
										<Route
											className={`w-5 h-5 mb-1 ${routeType === "loop" ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400"}`}
										/>
										<span
											className={`text-sm font-medium ${routeType === "loop" ? "text-blue-700 dark:text-blue-300" : "text-gray-700 dark:text-gray-300"}`}
										>
											{t("routeGenerator.routeType.loop", currentLanguage)}
										</span>
									</Label>
									<Label
										htmlFor="r1"
										className={`flex flex-col items-center justify-center p-3 rounded-md border ${routeType === "a-to-b" ? "bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800" : "border-gray-200 dark:border-gray-800"} cursor-pointer w-full h-full`}
									>
										<RadioGroupItem value="a-to-b" id="r1" className="sr-only" />
										<MapPin
											className={`w-5 h-5 mb-1 ${routeType === "a-to-b" ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400"}`}
										/>
										<span
											className={`text-sm font-medium ${routeType === "a-to-b" ? "text-blue-700 dark:text-blue-300" : "text-gray-700 dark:text-gray-300"}`}
										>
											{t("routeGenerator.routeType.aToB", currentLanguage)}
										</span>
									</Label>
								</RadioGroup>
							</div>
							...
							<div className="mt-5">
								<div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
									{routeType === "a-to-b"
										? t("routeGenerator.location.label.plural", currentLanguage)
										: t("routeGenerator.location.label.singular", currentLanguage)}
								</div>
								<div className="space-y-3">
									<div className="grid gap-1.5">
										<div className="flex items-center">
											...
											<Label htmlFor="startPointSearch" className="text-sm">
												{routeType === "a-to-b"
													? t("routeGenerator.location.startPoint", currentLanguage)
													: t("routeGenerator.location.startEndPointLoop", currentLanguage)}
											</Label>
										</div>
										<div className="flex items-center gap-2">
											...
											<Button
												...
												onClick={() => {
													if (userLocation) {
														setStartPoint({
															lat: userLocation[1],
															lng: userLocation[0],
															name: t("routeGenerator.location.currentLocationName", currentLanguage),
														});
													}
												}}
												...
												title={
													isUserLocationLoading
														? t("routeGenerator.location.useCurrent.fetching", currentLanguage)
														: userLocationError
															? t("routeGenerator.location.useCurrent.error", currentLanguage, {
																	userLocationError,
																})
															: !userLocation
																? t("routeGenerator.location.useCurrent.notAvailable", currentLanguage)
																: t("routeGenerator.location.useCurrent.use", currentLanguage)
												}
											>
												<LocateFixed size={16} />
											</Button>
									...
									<div className="h-20">
										{" "}
										{/* Changed from h-24 to h-20 (80px) */}
										{routeType === "a-to-b" && (
											<div className="grid gap-1.5">
												{" "}
												{/* End Point Content */}
												<div className="flex items-center">
													...
													<Label htmlFor="endPointSearch" className="text-sm">
														{t("routeGenerator.location.endPoint", currentLanguage)}
													</Label>
												</div>
												<div className="flex items-center gap-2">
													<div>
											...
											<div className="grid grid-cols-2 gap-3">
												...
												<div className="grid gap-1.5">
													...
													<div className="flex items-center">
														...
														<Label htmlFor="loopLengthKm" className="text-sm">
															{t("routeGenerator.loop.lengthLabel", currentLanguage)}
														</Label>
													</div>
													<Input
														id="loopLengthKm"
												...
												<div className="grid gap-1.5">
													...
													<div className="flex items-center">
														...
														<Label htmlFor="loopDirection" className="text-sm">
															{t("routeGenerator.loop.directionLabel", currentLanguage)}
														</Label>
													</div>
													<Select
														value={loopDirection}
														onValueChange={(value: string) => setLoopDirection(value as LoopDirection)}
													>
														<SelectTrigger id="loopDirection" className="border-gray-300 dark:border-gray-600">
															<SelectValue
																placeholder={t("routeGenerator.loop.directionPlaceholder", currentLanguage)}
															/>
														</SelectTrigger>
														<SelectContent>
															{loopDirections.map((dir) => (
																<SelectItem key={dir} value={dir}>
																	{dir === "ANY" ? t("routeGenerator.loop.directionAny", currentLanguage) : dir}
																</SelectItem>
															))}
														</SelectContent>
													</Select>
							...
							<div className="mt-5 pb-6">
								<div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
									{t("routeGenerator.surface.label", currentLanguage)}
								</div>
								<div className="grid gap-1.5">
									<div className="flex items-center">
										...
										<Label htmlFor="surfaceTypeGenModal" className="text-sm">
											{t("routeGenerator.surface.terrainTypeLabel", currentLanguage)}
										</Label>
									</div>
									<Select
										value={surfaceType}
										onValueChange={(value: string) => setSurfaceType(value as "paved" | "mixed" | "unpaved")}
									>
										...
										<SelectContent>
											<SelectItem value="unpaved">
												{t("routeGenerator.surface.option.unpaved", currentLanguage)}
											</SelectItem>
											<SelectItem value="mixed" disabled className="text-gray-400 dark:text-gray-500">
												{t("routeGenerator.surface.option.mixed", currentLanguage)}
											</SelectItem>
											<SelectItem value="paved" disabled className="text-gray-400 dark:text-gray-500">
												{t("routeGenerator.surface.option.paved", currentLanguage)}
											</SelectItem>
										</SelectContent>
									</Select>
								</div>
