...
export function RouteGeneratorModal({
  ...
  >(undefined);
  const [endPoint, setEndPoint] = useState<{ lat: number; lng: number; name: string } | undefined>(
    undefined,
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open: boolean) => !open && !isGenerating && onClose()}>
      <DialogContent
        className="p-0 w-[330px] sm:max-w-[525px] sm:w-[525px] border-r"
        hideCloseButton
      >
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 border-b border-amber-100 dark:border-amber-800/30 px-4 py-3 text-center">
          <p className="text-xs text-amber-800 dark:text-amber-400">
            <span className="font-semibold">
              {t("routeGenerator.betaNotice.title", currentLanguage)}
            </span>{" "}
                        ...
                        onClick={() => {
                          if (userLocation) {
                            setStartPoint({
                              ...
                              lng: userLocation[0],
                              name: t(
                                "routeGenerator.location.currentLocationName",
                                currentLanguage,
                              ),
                            });
                          }
                        ...
                        title={
                          isUserLocationLoading
                            ...
                            : userLocationError
                              ...
                              : !userLocation
                                ? t(
                                    "routeGenerator.location.useCurrent.notAvailable",
                                    currentLanguage,
                                  )
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="grid gap-1.5">
                          ...
                          <Input
                            ...
                            type="number"
                            placeholder={t(
                              "routeGenerator.loop.lengthPlaceholder",
                              currentLanguage,
                            )}
                        ...
                        <div className="grid gap-1.5">
                          ...
                          <Select
                            value={loopDirection}
                            onValueChange={(value: string) =>
                              setLoopDirection(value as LoopDirection)
                            }
                          >
                            <SelectTrigger
                              id="loopDirection"
                            ...
                            >
                              <SelectValue
                                placeholder={t(
                                  "routeGenerator.loop.directionPlaceholder",
                                  currentLanguage,
                                )}
                              />
                            </SelectTrigger>
                            <SelectContent>
                              {loopDirections.map((dir) => (
                                <SelectItem key={dir} value={dir}>
                                  {dir === "ANY"
                                    ? t("routeGenerator.loop.directionAny", currentLanguage)
                                    : dir}
                                </SelectItem>
                              ))}
                            </SelectContent>
              ...
              <div className="mt-5 pb-6">
                ...
                <div className="grid gap-1.5">
                  ...
                  <Select
                    value={surfaceType}
                    onValueChange={(value: string) =>
                      setSurfaceType(value as "paved" | "mixed" | "unpaved")
                    }
                  >
                    <SelectTrigger
                      id="surfaceTypeGenModal"
                      className="border-gray-300 dark:border-gray-600"
                    >
                      <SelectValue
                        placeholder={t(
                          "routeGenerator.surface.terrainTypePlaceholder",
                          currentLanguage,
                        )}
                      />
                    </SelectTrigger>
