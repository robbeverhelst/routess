  ...
  for (let i = 0; i < workingWaypoints.length - 1; i++) {
    ...
    if (workingDirectFlags[i + 1]) {
      ...
      try {
        ...
        if (!res.ok) {
          Logger.error(
            `[RCS/buildMixedRoute] API request failed with status ${res.status} for segment ${i}-${i + 1}`,
          );
        ...
        if (json && json.routes && json.routes[0]) {
          ...
          Logger.warn(
            `[RCS/buildMixedRoute] No route found or issue with API response for segment ${i}-${i + 1}. Falling back to direct.`,
          );
          if (!workingDirectFlags[i + 1]) {
        ...
        Logger.warn(
          `[RCS/buildMixedRoute] Network error for segment ${i}-${i + 1}, falling back to direct route: `,
          err,
        );
...
export const getRoute = async (
  ...
  if (allSegmentsRouted) {
    ...
    try {
      ...
      const waypointsString = currentWaypointsForAPI
...
> {
  ...
  Logger.info(
    `[RCS/calculateAtoBRoute] Fetching A-to-B route from: ${apiUrl.replace(accessToken, "<REDACTED_TOKEN>")}`,
  );
