...
import {
  ...
  type PopupInfo as MIMPopupInfo,
...
import {
  saveWaypointsToLocalStorage as saveWaypointsToStorage,
  loadWaypointsFromLocalStorage as loadWaypointsFromStorageService,
...
import {
  ...
  parseGPXFile,
  processGPXWaypoints,
  processHybridGPXData,
} from "@/features/routing/services/GPXService";
  ...
  try {
    ...
    if (parsedResult.waypoints) {
      ...
      if (parsedResult.trackPoints && parsedResult.trackPoints.length > 0) {
        Logger.info(
          `[routing.ts.importRouteFromGPX] Processing hybrid GPX: ${parsedResult.waypoints.length} waypoints + ${parsedResult.trackPoints.length} track points.`,
        );

        const processedResult = await processHybridGPXData(
          parsedResult.waypoints,
          parsedResult.trackPoints,
        );
        if (
          processedResult.error ||
          !processedResult.finalWaypoints ||
          !processedResult.finalDirectFlags
        ) {
          Logger.error(
            "[routing.ts.importRouteFromGPX] Error processing hybrid GPX data:",
            processedResult.error,
          );
        ...
        const { finalWaypoints: finalNewWaypoints, finalDirectFlags: newDirectFlags } =
          processedResult;
        ...
        if (currentRouteGeom && currentRouteGeom.length > 0) {
          ...
          Logger.info(
            `[routing.ts.importRouteFromGPX] Hybrid GPX imported: ${distanceKm} km, ${durationMinutes} min from exact track.`,
          );
        }
      } else {
        // Process GPX waypoints only (either from route points or smart-converted from track points)
        Logger.info(
          `[routing.ts.importRouteFromGPX] Processing ${parsedResult.waypoints.length} waypoints only.`,
        );
        ...
        if (
          ...
          Logger.error(
            "[routing.ts.importRouteFromGPX] Error processing GPX waypoints:",
            processedResult.error,
          );
            ...
            if (onError) {
              const errorMessage =
                error instanceof Error
                  ? error.message
                  : "Failed to calculate route after GPX import.";
      ...
      historySnapshot();
      Logger.info(
        `[routing.ts.importRouteFromGPX] Successfully imported ${getWaypoints().length} waypoints.`,
      );
    } else {
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error during GPX import process";
...
export const setRouteData = async (
  ...
  historySnapshot();
  Logger.info(
    `[setRouteData] Successfully set ${getWaypoints().length} waypoints. Final snapshot taken.`,
  );
};
...
export const setupRouting = async (
  ...
  // Return Promise because it's async
  ...
  try {
    ...
    if (loadedData) {
      ...
      if (getWaypoints().length >= 1) {
        const result: RouteResult = await getRouteFromService(
          map,
          accessToken,
          setRouteDistance,
          setRouteDuration,
        ...
        if (
          result.success &&
          result.waypointsSnapped &&
          result.snappedWaypoints &&
          result.snappedDirectFlags
        ) {
          ...
          Logger.info(
            "[routing.ts] Single waypoint loaded, no route to calculate yet or snapping failed.",
          );
        } else if (!result.success) {
          Logger.warn(
            "[routing.ts] Failed to calculate initial route by routing.ts. Service indicated failure.",
          );
        }
      }
    } else {
    ...
    Logger.error(
      "[routing.ts] Error loading waypoints from local storage in setupRouting by routing.ts:",
      error,
    );
  }
  ...
  const handleHistoryApplied = async (historyState: WaypointHistory) => {
    if (!_mapInstance || !_accessToken) {
      Logger.error(
        "[routing.ts.handleHistoryApplied] Map instance or access token not available for history event.",
      );
      return;
    }
    ...
    if (historyState.points.length >= 2) {
      ...
      if (
        ...
        Logger.info(
          "[routing.ts.handleHistoryApplied] Waypoints snapped during route recalculation after history change.",
        );
      } else if (!routeResult.success) {
        ...
        setHasRoute(false);
        Logger.warn(
          "[routing.ts.handleHistoryApplied] Route recalculation failed after history change.",
        );
      }
    } else {
      // 0 or 1 waypoint
      ...
      setRouteDuration("");
      setHasRoute(false);
      if (historyState.points.length === 0)
        updateWaypointsLayer(_mapInstance, [], _isMapLockedRef?.current ?? false); // Use ref value
    }
    ...
    Logger.info(
      "[routing.ts.handleHistoryApplied] History change processed. Points saved to storage:",
      finalPointsToSave.length,
    );
  };
...
export async function generateAndDisplayRouteAtoB(
  ...
  try {
    ...
    if (handleWaypointError)
      handleWaypointError(error instanceof Error ? error.message : "A critical error occurred.");
    setRouteDistance("");
    setRouteDuration("");
    setHasRoute(false);
    setIsRouteCoordsReady(false);
...
export async function generateAndDisplayRouteLoop(
  ...
  try {
    ...
    if (!loopResult.success || !loopResult.geometry) {
      ...
      loopResult = await generateSimplifiedLoop(startCoord, loopLengthKm, accessToken);
    }

    if (!loopResult.success || !loopResult.geometry) {
      if (handleWaypointError)
        handleWaypointError(loopResult.error || "Failed to generate natural loop.");
      setIsRouteCoordsReady(false);
      setHasRoute(false);
      setRouteDistance("");
      setRouteDuration("");
    ...
    try {
      ...
      if (routeResult.success) {
        ...
        historySnapshot();
        Logger.info(
          `[routing.ts] Natural loop generation successful. Created ${getWaypoints().length} waypoints.`,
        );
      } else {
        Logger.error("[routing.ts] Route calculation failed:", routeResult.error);
        if (handleWaypointError)
          handleWaypointError(
            routeResult.error || "Failed to calculate final route through waypoints.",
          );
        setIsRouteCoordsReady(false);
        setHasRoute(false);
        setRouteDistance("");
      ...
      if (handleWaypointError)
        handleWaypointError(
          error instanceof Error
            ? error.message
            : "A critical error occurred calculating the final route.",
        );
      setIsRouteCoordsReady(false);
      setHasRoute(false);
      setRouteDistance("");
    ...
    if (handleWaypointError)
      handleWaypointError(
        error instanceof Error
          ? error.message
          : "A critical error occurred during loop generation.",
      );
    setIsRouteCoordsReady(false);
    setHasRoute(false);
    setRouteDistance("");
...
async function generateNaturalLoop(
  ...
  Logger.info(
    `[routing.ts] Generating natural loop from ${startPoint}, target=${targetLengthKm}km, bearing=${initialBearing}°`,
  );
  ...
  Logger.info(
    `[routing.ts] Spider exploration phase using profile: ${profile}, target: ${targetLengthKm}km, scaling: ${scalingFactor}`,
  );
  ...
  const combinedDirections = [
    ...new Set([...clockDirections, ...cardinalDirections, ...recreationDirections]),
  ];
  ...
  for (let i = 0; i < directionsToTry.length; i++) {
    ...
    Logger.info(
      `[routing.ts] Trying circular loop with bearings: ${outboundBearing}° → ${secondBearing}° → ${thirdBearing}°`,
    );
    ...
    try {
      ...
      if (
        haversineDistance(startPoint, point1) < minDistanceBetweenPoints ||
      ...
      Logger.info(
        `[routing.ts] Getting multi-waypoint route through: start → p1 → p2 → p3 → p4 → p5 → start`,
      );
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        if (routes.length > 0) {
          ...
          if (tooLong || tooShort) {
            Logger.info(
              `[routing.ts] Rejecting route: ${(bestRoute.distanceMeters / 1000).toFixed(1)}km - ${tooLong ? "too long" : "too short"} vs target ${targetLengthKm}km`,
            );
            continue;
          }
          ...
          if (tooMuchOverlap) {
            Logger.info(
              `[routing.ts] Rejecting route: Too much road overlap (${(bestRoute.overlapRatio * 100).toFixed(0)}%)`,
            );
            continue;
          }
          ...
          const startDistance = haversineDistance(
            startPoint,
            bestRoute.geometry[bestRoute.geometry.length - 1],
          );
          if (startDistance > 0.2) {
            // More than 200m from the start point
            Logger.info(
              `[routing.ts] Rejecting route: Doesn't return to start (${startDistance.toFixed(3)}km away)`,
            );
            continue;
          }

          Logger.info(
            `[routing.ts] Found route: ${(bestRoute.distanceMeters / 1000).toFixed(1)}km, score=${bestRoute.score.toFixed(2)} (length_ratio=${bestRoute.lengthRatio.toFixed(2)}, overlap=${(bestRoute.overlapRatio * 100).toFixed(0)}%)`,
          );
          ...
          if (!bestLoop || bestRoute.score < bestLoop.score) {
            ...
            if (bestRoute.score < 0.15 && bestRoute.overlapRatio < 0.2) {
              Logger.info(
                `[routing.ts] Found excellent loop match with low overlap, using it immediately`,
              );
              break;
            }
          }
  ...
  if (!bestLoop) {
    ...
    try {
      ...
      Logger.info(
        `[routing.ts] Enhanced rectangular approach: start → p1 → p2 → p3 → p4 → p5 → p6 → start`,
      );
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        const tooShort = actualDistance < targetLengthKm * 0.5;

        if (tooLong || tooShort) {
          Logger.info(
            `[routing.ts] Rectangular route ${tooLong ? "too long" : "too short"}: ${actualDistance.toFixed(1)}km vs ${targetLengthKm}km, rejecting`,
          );
        ...
        if (startDistance > 0.2) {
          // More than 200m from the start point
          Logger.info(
            `[routing.ts] Rectangular route doesn't return to start (${startDistance.toFixed(3)}km away), rejecting`,
          );
