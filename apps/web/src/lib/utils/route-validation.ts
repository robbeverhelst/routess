    ...
    for (let i = 0; i < validWaypoints.length - 1; i++) {
      for (let j = i + 1; j < validWaypoints.length; j++) {
        ...
        const distance = Math.sqrt(
          Math.pow((lon2 - lon1) * 111320 * Math.cos((lat1 * Math.PI) / 180), 2) +
            Math.pow((lat2 - lat1) * 110540, 2),
        );
 ...
 */
export const validateDirectFlags = (
  directFlags: unknown,
  ...
  if (directFlags.length !== waypointCount) {
    errors.push(
      `Direct flags length (${directFlags.length}) must match waypoints length (${waypointCount})`,
    );
  }
