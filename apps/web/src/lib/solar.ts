 ...
 */
export function getSolarPositionForTimeOfDay(
	...
	return getSolarPosition(calculationDate, latitude, longitude);
}
...
export function getSolarPosition(date: Date, latitude: number, longitude: number): SolarPosition {
	...
	const elevation =
		(Math.asin(
			Math.sin(declination) * Math.sin(latRad) + Math.cos(declination) * Math.cos(latRad) * Math.cos(hourAngle),
		) *
			180) /
		Math.PI;
