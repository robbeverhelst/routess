...
function isMapAssetRequest(request) {
  return (
    MAP_ASSET_PATTERNS.some((pattern) => pattern.test(request.url)) && !isMapTileRequest(request)
  );
}
...
function createRouteResponse(geometry, waypoints, isFromCache) {
    ...
    routes: [
      {
        ...
        legs: waypoints.slice(0, -1).map((waypoint, i) => {
          ...
          const legDistance =
            haversineDistance(waypoint[1], waypoint[0], nextWaypoint[1], nextWaypoint[0]) * 1000;
          return {
  ...
  console.log(
    `[SW] Generated ${isFromCache ? "cached" : "direct"} offline route:`,
    offlineResponse,
  );
