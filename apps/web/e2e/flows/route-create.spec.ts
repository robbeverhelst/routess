...
// TODO(e2e): these specs require the Mapbox map to fully load (the bridge only
// mounts after `map.load` fires). In headless chromium the Mapbox WebGL
// pipeline currently errors out before firing `load`. Marked `test.fixme` for
// now; will re-enable once we resolve headless WebGL or switch to a Mapbox
// double for the bridge-dependent assertions.

test.describe
	.fixme("hand-drawn route", () => {
