...
const loadHistoryFromStorage = (): { undo: WaypointHistory[], redo: WaypointHistory[] } => {
  try {
    ...
    Logger.info('[HistoryManager] Loaded history from storage. Undo:', undo.length, 'Redo:', redo.length);
    return { undo, redo };
  } catch (error) {
...
const saveHistoryToStorage = () => {
  try {
    ...
    if (import.meta.env.DEV) {
      Logger.info('[HistoryManager] Saved history to storage. Undo:', undoStack.length, 'Redo:', redoStack.length);
    }
  } catch (error) {
...
if (import.meta.hot) {
  import.meta.hot.dispose(() => {
    Logger.info('[HistoryManager.ts] HMR disposing old instance. Current history will be saved by modification functions if needed, or reloaded on accept.');
  });
...
export const snapshot = () => {
  ...
  if (undoStack.length > 0) {
    ...
    if (
      ...
      Logger.info('[HistoryManager snapshot] New state is identical to current top of undo stack. Skipping snapshot.');
      return; // Don't push identical state
    }
  }
  
  if (import.meta.env.DEV) {
    Logger.info('[HistoryManager snapshot] Creating snapshot. Waypoints:', JSON.stringify(currentWaypointsSnapshot), 'Flags:', JSON.stringify(currentFlagsSnapshot));
    Logger.info('[HistoryManager snapshot] Current undoStack length BEFORE push:', undoStack.length);
  }
  ...
  if (redoStack.length > 0) {
    Logger.info('[HistoryManager snapshot] Clearing redoStack due to new snapshot. Redo count was:', redoStack.length);
    redoStack = [];
  }
  
  saveHistoryToStorage(); 
  if (import.meta.env.DEV) {
    Logger.info('[HistoryManager snapshot] Final undoStack length:', undoStack.length, 'redoStack cleared and history saved.');
  }
};
...
export const internalDoUndo = (): WaypointHistory | null => {
  ...
  Logger.info('[HistoryManager internalDoUndo] State to apply after undo:', JSON.stringify(stateToApply));
  return stateToApply;
};
...
export const stepBack = async (): Promise<void> => {
  ...
  Logger.info('[HistoryManager.stepBack] Undo applied, event emitted. History saved by internalDoUndo.');
};
...
export const stepForward = async (): Promise<void> => {
  ...
  Logger.info('[HistoryManager.stepForward] Redo applied, event emitted. History saved by internalDoRedo.');
}; 