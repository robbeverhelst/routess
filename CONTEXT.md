...
**NavigationSession**:
The device-local, screen-on act of following a Route's (or ExternalRoute's) **RoutePath** with live positional guidance: a banner and voice announcing **Cues**, a follow camera, and **Rejoin** when the user strays. Consumes geometry, never Waypoints, so saved Routes, RouteDrafts, ExternalRoutes, and imports are all navigable through the same pipeline. States: `following`, `offRoute`, `rejoining`, `ended`. A NavigationSession never mutates the Route, and it is not activity recording (a future, separate concept that may run during a session). Survives an app restart via a resume prompt; exists only on the device, never persisted server-side. Distinct from **LocationTracking**, which is just the presentational GPS puck outside any session.
_Avoid_: trip, ride, activity, navigation mode.

**Cue**:
One guidance unit of a NavigationSession, anchored to a position along the RoutePath: what the banner shows and the voice speaks, in the app locale. Two kinds: a **ManeuverCue** (street-level, produced by map-matching the geometry through the routing engine: "turn right onto Schelde dijkpad") and a **NodeCue** (produced by decorating the geometry with **NodeNetwork** passages: "at node 47, head toward 52"). NodeCues decorate, never replace: when both are near, the NodeCue wins banner and voice and the ManeuverCue remains as fallback. Cues are fetched once at session start and cached with the route for offline use; a stretch that cannot be map-matched gets a synthesized "follow the path" Cue rather than failing the session.
_Avoid_: instruction, step, turn, maneuver (reserved for the raw routing-engine payload a ManeuverCue is derived from).

**Rejoin**:
What "rerouting" means in a NavigationSession: a routed connector from the user's current position to the nearest not-yet-ridden point of the remaining RoutePath, spliced in so the planned Route stays the thing being navigated. Never a re-plan to the destination (which would discard the planned loop or detour). Triggered automatically after a sustained off-route condition, or manually. Requires connectivity; offline, the session degrades to an honest bearing-and-distance indicator back to the RoutePath.
_Avoid_: reroute and recalculate in code (UI copy may say "rerouting"); never confuse with the planner's "recalculate".

## Editing & state

**RouteDraft**:
A Route-in-progress: a document holding waypoints, per-waypoint Type, RoutePath, Distance, Duration, ElevationGain, an activity, and **routingPreferences**. In the web app, the RouteDraft is the working state of `routingStore`; for CLI and agent clients it is the same shape recomputed via `/v1/routing/route` and persisted via the routes API (ADR 0034; the older `/v1/draft/*` endpoints no longer exist). A RouteDraft has a **mode**: `unsaved` (composing fresh; on save becomes a new Route) or `editing(routeId, baseline)` (bound to a saved Route; carries a snapshot of the saved fields and on save PATCHes that Route). The current **activity** is a property of the RouteDraft; the global activity setting is just a default applied when starting a new draft, never overwritten by loading a Route. The same rule applies to **routingPreferences**: per-Activity user defaults are copied onto the draft at creation; from then on they travel with the draft and are persisted on save. A RouteDraft is never persisted on the server side; it lives only in the client that holds it (browser store, CLI invocation, agent context).
